...
const Text3DFlip = ({
  ...
  const text = useMemo(() => {
    try { return extractTextFromChildren(children) } catch { return "" }
  }, [children])
  ...
  const charOffsets = useMemo(() => {
    const offsets = [0]
    for (const word of characters) offsets.push(offsets.at(-1)! + word.characters.length)
    return offsets
  }, [characters])
  ...
  const handleHoverStart = useCallback(async () => {
    ...
    try {
      ...
      await animate(".text-3d-flip-char", { transform: rotationTransform }, { ...transition, delay: (i: number) => delays[i] })
      if (!isMountedRef.current) return
      await animate(".text-3d-flip-char", { transform: "rotateX(0deg) rotateY(0deg)" }, { duration: 0 })
    } finally {
      if (isMountedRef.current) isAnimatingRef.current = false
    }
...
const CharBox = memo(({ char, textClassName, flipTextClassName, rotateDirection }: CharBoxProps) => (
  <span className="text-3d-flip-char inline transform-3d" style={{ transform: CONTAINER_TRANSFORMS[rotateDirection] }}>
    <span className={cn("relative h-[1lh] backface-hidden", textClassName)} style={{ transform: FRONT_FACE_TRANSFORMS[rotateDirection] }}>{char}</span>
    <span className={cn("absolute top-0 left-0 h-[1lh] backface-hidden", flipTextClassName)} style={{ transform: SECOND_FACE_TRANSFORMS[rotateDirection] }}>{char}</span>
  </span>
))
